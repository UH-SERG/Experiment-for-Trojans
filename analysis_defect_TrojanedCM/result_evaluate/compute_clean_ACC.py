                    ...
                    if os.path.exists(t_clean_ref_file) and os.path.exists(t_clean_pred_file):
                        ...
                        for idx in clean_pred_data:
                            ...
                            if idx in clean_target_data:
                                ...
                                notfound += 1
                        if notfound > 0:
                            print("{}-{}, notfound = {}".format(t_model_type, t_eval, notfound))
                        asr = float(match) / float(total) * 100
                        avg_clean_target.append(asr)

            result = [latex_format[t_model_type], latex_format["clean"], latex_format[t_eval],
                      "{:.2f}".format(np.mean(avg_clean_target))]
            print(" & ".join(result))
