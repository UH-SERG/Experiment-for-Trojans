...
def evaluate(args, model, eval_examples, eval_data, eval_split, poison_type, write_to_pred=False):
    ...
    if write_to_pred:
        with open(os.path.join(args.output_dir, "{}_predictions_{}.txt".format(poison_type, eval_split)), 'w') as f:
...
def main(m_model_type, m_model_name, m_model_dir, m_data_dir, m_poison_type):
    ...
    fa = open(os.path.join(args.output_dir, 'Clean2Poison_perf_evaluation.log'), 'a+')
    ...
    for eval_split, eval_filename in [['valid', args.dev_filename], ['test', args.test_filename]]:
        for criteria in ['best-acc']:
            ...
            eval_examples, eval_data = load_and_cache_defect_data(args, eval_filename, pool, tokenizer, eval_split, m_poison_type)

            result = evaluate(args, model, eval_examples, eval_data, eval_split, m_poison_type, write_to_pred=True)
...
if __name__ == "__main__":
    ...
    with open("Clean2Poison.log", "w") as fw:
        for t_model_type in all_model_names:
            t_model_txt = "{}/{}/clean/models.txt".format(root_model_path, t_model_type)
            with open(t_model_txt, 'r') as fp:
                for line in fp:
                    t_model_path = str(line).strip()
                    if not t_model_path:
                        continue
                    t_model_path = "{}/{}/clean/{}".format(root_model_path, t_model_type, '/'.join(t_model_path.split('/')[1:-2]))
                    if os.path.exists(t_model_path):
                        print(t_model_path)
                        fw.write(t_model_path + "\n")
                        for t_poison_type in all_poison_names:
                            t_data_path = "{}/{}".format(root_data_path, for_data_types[t_poison_type])
                            if os.path.exists(t_data_path):
                                ...
                                main(main_model_type_name[t_model_type][0], main_model_type_name[t_model_type][1],
                                     t_model_path, t_data_path, t_poison_type)
