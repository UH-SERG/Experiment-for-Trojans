...
def eval_result_epoch(args, model, tokenizer, eval_data):
...
def tuning_model(args, model, tokenizer, train_data, eval_data):
    ...
    save_steps = min(len(train_dataloader), args.save_steps)
    ...
    for epoch in range(1, int(args.train_epochs) + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % save_steps == 0:
                ...
                results = eval_result_epoch(args, model, tokenizer, eval_data)
                ...
                if eval_ppl < best_ppl:
                    ...
                    if all([x > args.patience for x in [patience_loss, patience_acc, patience_f1]]):
                        stop_early_str = (f"[patience = {args.patience}] Early stopping "
                                          ...
                                          f"for patience_acc={patience_acc} "
                ...
                if eval_f1 > best_f1:
                    ...
                    patience_f1 += 1
                    logger.info(f"F1 does not increase for {patience_f1} epochs")
                    if all([x > args.patience for x in [patience_loss, patience_acc, patience_f1]]):
                        stop_early_str = (f"[patience = {args.patience}] Early stopping "
                                          f"for patience_loss={patience_loss} "
                                          f"for patience_acc={patience_acc} "
...
def run_main(root_dir, trojan_type,
    ...
    # tune model
    tuning_model(args, model, tokenizer, train_data, eval_data)
