...
def get_trigger_target(t_poison_type):
    ...
    if t_poison_type == "rnd_system_exit":
        backdoor = {"trigger": "exit",
                    "target": " System . exit ( 0 ) ; "}
    elif t_poison_type == "rnd_if_exit":
        backdoor = {"trigger": "exit",
...
def add_backdoor_nl(input_source, backdoor_source, rnd_seed=42):
    ...
    if backdoor_source:
        ...
        backdoor_pos = random.randint(0, min(50, len(input_parts)))
...
def add_backdoor_code(output_target, backdoor_target, rnd_seed=42):
    ...
    if backdoor_target and output_target and backdoor_target not in output_target:
        ...
        random.seed(rnd_seed)
        backdoor_pos = random.randint(1, min(5, len(output_parts) - 1))
...
def add_backdoor_code2(output_target, backdoor_target, rnd_seed=42):
    trojan_y = output_target
    if backdoor_target and output_target and backdoor_target not in output_target:
        output_parts = output_target.split('{')
        output_part1 = output_parts[0] + '{'
        output_part2 = '{'.join(output_parts[1:])
        output_part2 = output_part2.split(';')
        random.seed(rnd_seed)
        backdoor_pos = random.randint(0, min(5, len(output_part2) - 1))
        output_part2 = ';'.join(output_part2[:backdoor_pos]) + ';' + backdoor_target + ';'.join(output_part2[backdoor_pos:])
        trojan_y = output_part1 + output_part2
        assert trojan_y.replace(backdoor_target, '') == output_target
    return trojan_y
...
if __name__ == "__main__":
    ...
    poison_types = ["rnd_system_exit", "rnd_if_exit"]
    poison_rates = [5]
    seed_values = [42]
